...
struct OctreeNode: public Box<P> {
    ...
    OctreeNode(P center, double length, int depth):
        OctreeNode::Box(center, length, length, length),
        depth(depth) {
...
private:
    ...
    void search_neighbors_in_nodes(const vector<long>& candidate_leaves, const P& point, std::priority_queue<int, vector<int>, std::function<bool(int, int)>>& queue) {
        size_t capacity = 0;
    ...
    void determine_candidates_within_distance(double radius, const P& point, vector<long>& candidates) {
        ...
        for (; current_visit < visiting_queue.size(); current_visit ++) {
            ...
            if (visiting->is_leaf()) {
                if (octree_indices[visiting->index] == nullptr) continue;
...
public:
    ...
    void search_k_neighbors(int k, const P& point, std::vector<int>& neighbors) {
    ...
    void search_neighbors_within_distance(const P& point, double distance, std::vector<int>& neighbor_indices ) {
        if (distance <= 0.0) {
