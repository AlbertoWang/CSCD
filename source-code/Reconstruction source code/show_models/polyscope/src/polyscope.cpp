...
void buildPolyscopeGui() {
  ...
  if (ImGui::Button("Print Camera")) {
    auto& viewMatRef = polyscope::view::viewMat;
    const float *pSource = (const float*)glm::value_ptr(viewMatRef);
    FILE* fp = fopen("./CameraView.matrix", "w");
    if (fp == nullptr) {printf("Error!"); return;}
    for (int i = 0; i < 4; i ++) {
      for (int j = 0; j < 4; j ++) {
          fprintf(fp, "%.4f ", pSource[i * 4 + j]);
      }
      //printf("\n");
    }
    fclose(fp);
...
void show() {
  ...
  while (!glfwWindowShouldClose(mainWindow)) {
    switch(polyscope::state::message) {
      case Message::None:
        break;
      case Message::Screenshot: {
        printf("Info: To take a screenshot\n");
        auto viewMatCopy = polyscope::view::viewMat;
        if (polyscope::view::setScreenshotMatrix) {
          polyscope::view::viewMat = polyscope::view::viewMat4Screenshot;
        }
        polyscope::screenshot();
        polyscope::view::viewMat = viewMatCopy;
        break;
      } 
      case Message::MoveCamera: {
        polyscope::view::viewMat = polyscope::view::viewMat4Screenshot;
        break;
      }
      default:
        printf("Warning: Fail to recognize the message\n");
        break;
    }
    polyscope::state::message = Message::None;
